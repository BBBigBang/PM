    ...
    if len(key) == 1:
        ...
        for tag in tags:
            if deps.has_key((tag[0], tag[1])) and (deps[(tag[0], tag[1])] == 'compound' or \
                ...
                if tag[0] == path[1] and tag[0] == path[2]:
                    key = [tag[0], tag[1]]
                else:
                    key = [tag[1], tag[0]]
    return mod1, key, mod2
            ...
            if deps.has_key((tag[0], tag[1])) and (deps[(tag[0], tag[1])] == 'compound' or \
                ...
                if abs(int(tag[0]) - int(tag[1])) == 1 or deps[(tag[0], tag[1])] == 'neg':
                    if deps[(tag[0], tag[1])] == 'neg':
                        key = [tag[0], tag[1]]
                        break
                    elif tag[0] == path[2] or tag[0] == path[3]:
                        key = [tag[0], tag[1]]
                    else:
                        key = [tag[1], tag[0]]
            ...
            if deps.has_key((tag[0], tag[1])) and (deps[(tag[0], tag[1])] == 'compound' or \
                ...
                if abs(int(tag[0]) - int(tag[1])) == 1 or deps[(tag[0], tag[1])] == 'neg':
                    if deps[(tag[0], tag[1])] == 'neg':
                        key = [tag[0], tag[1]]
                        break
                    elif tag[0] == path[1] or tag[0] == path[2]:
                        key = [tag[0], tag[1]]
                    else:
                        key = [tag[1], tag[0]]
            elif deps.has_key((tag[1], tag[0])) and (deps[(tag[1], tag[0])] == 'compound' or \
...
def processing_GAD(pattern, mod1, key, mod2, e1_mods, e2_mods, htm_present, e1, e2, e1_ext, e2_ext, rest_ext, path, graph, \
    inst, sent, deps, deps_dir, dep_fs, mod_dep):
    # read the dictionary
    dir = home_dir + 'files/'
    GAD_keywords = set()
    for line in open(dir + 'GAD.words'):
        GAD_keywords.add(line.strip())
    common_words = set()
    ...
    if len(key) != 0:
        if pattern != 'pattern5':
            ...
            if (sent[key[0]].lower() in GAD_keywords) or (sent[key[0]].lower() in common_words):
                mod1s, mod2s, sub_key = htm.supplement(sent, e1, e2, graph, deps_dir, mod1, key[0], mod2)
                htm_present.append(htm.htm_represent(sent, e1, e2, mod1s, mod2s, key, sub_key))
                return key
            else:
                dic_key = dic_key_word(e1, e2, path, sent, GAD_keywords, common_words)
                if dic_key is not None:
                    ...
                    dic_key = dic_key_word(e1, e2, e1_mods + e2_mods, sent, GAD_keywords, common_words)
                    if dic_key is not None:
                        mod1s, mod2s, sub_key = htm.supplement(sent, e1, e2, graph, deps_dir, None, dic_key, None)
                        htm_present.append(htm.htm_represent(sent, e1, e2, mod1s, mod2s, dic_key, sub_key))
                        key = []
                        ...
                        dic_key = dic_key_word(e1, e2, e1_ext + e2_ext + rest_ext, sent, GAD_keywords, common_words)
                        if dic_key is not None:
                            mod1s, mod2s, sub_key = htm.supplement(sent, e1, e2, graph, deps_dir, None, dic_key, None)
                            htm_present.append(htm.htm_represent(sent, e1, e2, mod1s, mod2s, dic_key, sub_key))
                            key = []
...
def processing_GAD_2(pattern, mod1, key, mod2, e1_mods, e2_mods, htm_present, e1, e2, e1_ext, e2_ext, rest_ext, path, graph, \
    inst, sent, deps, deps_dir, dep_fs, mod_dep):
    # read the dictionary
    dir = home_dir + 'files/'
    GAD_keywords = set()
