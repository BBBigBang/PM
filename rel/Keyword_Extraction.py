    ...
    for line in open(filename):
        ...
        if whitespace_index % 2 == 0:
            ...
            if whitespace_index != 0:
                if len(dep) == 0 and RPT:
                    dep_list.append(copy.deepcopy(dep_pre))
                    dep_dir_list.append(copy.deepcopy(dep_dir_pre))
                    dep_f_list.append(copy.deepcopy(dep_f_pre))
                    mod_dep_list.append(copy.deepcopy(mod_dep_pre))
                else:
                    dep_list.append(dep)
            ...
            if line.strip().lower() == 'rpt/nnp':
                sent_list.append(copy.deepcopy(sent_pre))
                pos_list.append(copy.deepcopy(pos_pre))
                RPT = True
            else:
                sent = ['ROOT']
                pos = ['ROOT']
...
def extraction_pattern_just(list_indexs, dep_file, out_htm):
    ...
    FileUtil.writeStrLines(out_htm, htm_present)
    
    return nodes_list, edge_list, triple_list

def extraction_mid_version(list_indexs, dep_file, out_htm):
    initial_filtered_deps()
    sent_list, pos_list, dep_list, dep_dir_list, dep_f_list, mod_dep_list = read_dep_file(dep_file)
        ...
        if len(sent) > 3:
            ...
            if path is not None:
                ...
                if key == None:
                    key = []
                if len(key) != 0:
                    output_json.gen_jsonData(inst, types, sent, key, nodes_list, edge_list, sentence_score[entity_type_index], \
                        entity_list, entity_id_list, concept_ids[entity_type_index], relation_list)
                    str_key = output_json.get_keywords(key, sent)
                    old_indexs = [int(index) for index in old_offset[entity_type_index].split(' ')]
                    old_indexs.append(str_key)
                    triple_list.append(old_indexs)
...
def extraction(list_indexs, dep_file, out_htm):
    ''' processing start...
    '''
    initial_filtered_deps()
    sent_list, pos_list, dep_list, dep_dir_list, dep_f_list, mod_dep_list = read_dep_file(dep_file)
